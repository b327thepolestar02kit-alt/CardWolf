...
- The visible version in the first screen is explicitly v68 and is also refreshed by version-check.js before app.js runs.
...
Deployment
- Replace the files in the GitHub Pages repository with the contents of the v68 folder.
- Keep the existing images folder if it already exists in the repository.
- Confirm the first screen shows v68 before testing other functions.
...
- Version references and cache-busting query parameters are synchronized to v68.
- No separate images folder is included because no image assets were changed in this release.


[v68追加カード]
カードプールを60種類に拡張しました。新規20種はcards.config.jsonに追加済みです。PCでprepare_cards.cmdを実行するとYGOPRODeckから画像とカードデータをimages/およびdata/cards.jsへ再生成できます。
